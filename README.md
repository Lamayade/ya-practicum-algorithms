...
Решение через рекурсию подразумевает граничный случай, когда массив
состоит из одного человека. Единственное отличие - в рекурсивную функцию
в качестве параметра нужно передавать текущий индекс, поскольку каждый раз 
считалка идёт по кругу, а не от первого человека.
...
3 1 | 0 2 | 6 | 5 4 | 7  __Так нельзя: после сортировки блоков их не объединить в отсортированный массив.__


3 | 1 0 2 | 6 5 4 7  __И так нельзя.__


3 1 0 2 | 6 5 4 | 7  __А вот такое деление - в самый раз!__

0 1 2 3 | 4 5 6 | 7 __Сортируем блоки:__


0 1 2 3 4 5 6 7 __Объединяем блоки:__
...
Алгоритм сортировки состоит из трёх шагов:
    ...
    Разбить исходную последовательность на k блоков.
    Блоки могут иметь разные размеры.
    Первый блок обязательно должен содержать 0.
    Если длина первого блока — r элементов,
    то максимальным значением в первом блоке
    должно быть число r - 1.
    А следующий блок (если он вообще будет)
    должен содержать число r.
...
## Решение через цикл
Пойдем по каждому элементу и сравним его значение __x__ и индекс __i__.
Если __n = x - i > 0__, то этот и следующие __n__ блоков - это один блок.
